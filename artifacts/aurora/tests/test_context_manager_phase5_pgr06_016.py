"""
Mission 016 Phase 5 — Gated Activation PGR-06 ONLY (REGRA 25 / REGRA 27).

Proves:
  - Repo default OFF / 0%; PGR-06 not armed
  - PGR-06 (100% / STAGE6_100PCT) path when explicitly flagged
...
from src.conversation.progressive_gate_review import (
    ...
    operator_enable_pgr06_instructions,
    pgr01_enable_flag,
    pgr02_enable_flag,
    pgr03_enable_flag,
    pgr04_enable_flag,
...
def _prior_ctx() -> dict:
    return {
        "session_id": "pgr06-test-session-001",
        "last_home": "Flamengo",
        "last_away": "Palmeiras",
...
# ---------------------------------------------------------------------------
# Defaults / PGR-06 independent gate
# ---------------------------------------------------------------------------


def test_pgr06_defaults_off_not_armed():
    _clear_flags()
    ...
    assert snap["pgr06_enable"] is False
    ...
    assert PGR06_STAGE == "STAGE6_100PCT"
...
def test_pgr06_pct100_without_pgr06_remains_off():
    """Independent gate: funnel pct=100 alone must NOT activate without PGR-06."""
    _clear_flags()
    os.environ["AURORA_SOLE_WRITER_FUNNEL_PCT"] = "100"
    os.environ["ENABLE_STS_WRITE_FUNNEL_BOUNDARY"] = "1"
    os.environ["ENABLE_STS_WRITE_FUNNEL_ANALYZE"] = "1"
    ...
    try:
        ...
        assert pgr_metrics_snapshot()["pgr06_blocked_missing_flag"] >= 1
    finally:
        _clear_flags()


def test_pgr06_hundred_pct_path_when_enabled():
    _clear_flags()
    _arm_pgr06()
    try:
        assert pgr06_enable_flag() is True
        assert require_pgr06_for_stage6() is True
        assert get_funnel_pct() == 100
        assert funnel_stage_name() == "STAGE6_100PCT"
        assert note_subject_funnel_enabled() is True
        assert analyze_funnel_enabled() is True  # prior stage retained at 100%
...
def test_pgr06_prior_stages_live_when_prior_gates_also_armed():
    """At 100%, boundary + analyze + note remain available when prior gates armed."""
    _clear_flags()
    _arm_pgr06()
    os.environ["AURORA_PGR_01_ENABLE"] = "1"
    os.environ["AURORA_PGR_02_ENABLE"] = "1"
...
def test_pgr06_c17_commit_note_subject_when_armed():
    _clear_flags()
    _arm_pgr06()
    reset_funnel_metrics()
    ctx = _prior_ctx()
    try:
        ...
        assert res.pct == 100
        assert res.stage_name == "STAGE6_100PCT"
...
def test_pgr06_canary_selects_all_sessions_at_100():
    """At 100%, canary bucket always selects (no residual OFF cohort)."""
    _clear_flags()
    _arm_pgr06()
    try:
        from src.conversation.sole_writer_funnel import in_funnel_canary_bucket

        assert in_funnel_canary_bucket("any-session-key", force=False) is True
        assert in_funnel_canary_bucket("", force=False) is True
    finally:
        _clear_flags()


def test_pgr06_rollback_to_off():
    _clear_flags()
    _arm_pgr06()
    assert get_funnel_pct() == 100
    ...
    assert get_funnel_pct() == 0
    assert note_subject_funnel_enabled() is False
    assert out["pgr06_enable"] is False
    assert pgr_metrics_snapshot()["pgr06_rollback"] >= 1
    _clear_flags()


def test_pgr05_still_coherent_when_pgr06_off():
    """Prior gate remains available after PGR-06 rollback / when PGR-06 unset."""
    _clear_flags()
    _arm_pgr06()
    rollback_pgr06_to_off()
    _arm_pgr05()
...
def test_pgr04_still_coherent_when_pgr06_off():
    _clear_flags()
    _arm_pgr06()
    rollback_pgr06_to_off()
    _arm_pgr04()
...
def test_pgr03_still_coherent_when_pgr06_off():
    _clear_flags()
    _arm_pgr06()
    rollback_pgr06_to_off()
    _arm_pgr03()
...
def test_pgr02_still_coherent_when_pgr06_off():
    _clear_flags()
    _arm_pgr06()
    rollback_pgr06_to_off()
    _arm_pgr02()
...
def test_pgr01_still_coherent_when_pgr06_off():
    _clear_flags()
    _arm_pgr06()
    rollback_pgr06_to_off()
    _arm_pgr01()
...
def test_pgr06_funnel_rollback_alone_clears_pct():
    _clear_flags()
    _arm_pgr06()
...
# ---------------------------------------------------------------------------
# No auto-advance / Phase 6 locked / fail-closed without enable
# ---------------------------------------------------------------------------


def test_pgr06_enable_with_pct50_does_not_auto_jump_to_100():
    """PGR-06 armed + pct=50 stays on PGR-05 path (no auto-advance to 100%)."""
    _clear_flags()
    os.environ["AURORA_PGR_05_ENABLE"] = "1"
    os.environ["AURORA_PGR_06_ENABLE"] = "1"
    os.environ["AURORA_SOLE_WRITER_FUNNEL_PCT"] = "50"
...
def test_pgr06_po_unlock_alone_does_not_unlock_without_pgr06():
    """PO unlock alone must not activate 100% without PGR-06 enable."""
    _clear_flags()
...
def test_pgr06_shadow_still_works_with_gates_off():
    _clear_flags()
...
def test_pgr06_legacy_writers_present():
    _clear_flags()
...
def test_pgr06_flag_snapshot_and_runbook():
    _clear_flags()
    snap = flag_snapshot()
    assert "progressive_gate_review" in snap
    assert snap["progressive_gate_review"]["pgr06_enable"] is False
    assert snap["AURORA_PGR_06_ENABLE"] is False
    assert snap["production_write_active"] is False
    assert assert_legal_flag_matrix() == []
    runbook = operator_enable_pgr06_instructions()
    assert "AURORA_PGR_06_ENABLE=1" in runbook
    assert "AURORA_SOLE_WRITER_FUNNEL_PCT=100" in runbook
    ...
    assert "rollback_pgr06_to_off" in runbook
...
def test_pgr06_rollback_pgr05_helper_still_works():
    _clear_flags()
    _arm_pgr05()
    assert get_funnel_pct() == 50
    rollback_pgr05_to_off()
