...
def test_pgr01_defaults_off_not_armed():
    ...
    assert snap["pgr02_not_started"] is True
    assert snap["auto_advance"] is False
    assert snap["rollback_possible"] is True
    assert PGR01_PCT == 1
    assert PGR01_STAGE == "EM_STAGE1_SOLE_PATH_1PCT"
    assert AUTHORIZED_HIGHEST_GATE == "PGR-01"
    assert AUTHORIZED_OPERATIONAL_MAX_PCT == 1
    ...
    assert flag_snap["phase5_pgr02_not_started"] is True
...
# ---------------------------------------------------------------------------
# Higher pct / PGR-02 locked
# ---------------------------------------------------------------------------


def test_pgr01_pct_above_1_fail_closed():
    _clear_em_flags()
    os.environ["ENABLE_EM_PGR_01"] = "1"
...
def test_pgr02_not_unlocked_when_flag_set():
    """PGR-02 flag armed during PGR-01-only mission → higher gate blocked."""
    _clear_em_flags()
    _arm_pgr01()
    ...
    try:
        ...
        assert get_effective_em_activation_pct() == 0
        assert require_em_pgr01() is False
        assert em_pgr_flag_snapshot()["pgr02_not_started"] is True
        assert em_pgr_metrics_snapshot()["em_pgr_higher_gate_blocked"] >= 1
    finally:
        _clear_em_flags()


def test_pgr02_enable_alone_does_not_unlock_5pct():
    _clear_em_flags()
    os.environ["ENABLE_EM_PGR_02"] = "1"
    os.environ["EM_ACTIVATION_PCT"] = "5"
    os.environ["ENABLE_EXECUTION_MANAGER"] = "1"
...
def test_no_auto_advance_and_runbook():
    ...
    try:
        ...
        assert "PGR-02" in runbook or "ENABLE_EM_PGR_02" in runbook
    finally:
        _clear_em_flags()
