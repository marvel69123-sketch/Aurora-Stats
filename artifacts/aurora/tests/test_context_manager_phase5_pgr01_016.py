"""
Mission 016 Phase 5 — Gated Activation PGR-01 ONLY (REGRA 25).

Proves:
  - Repo default OFF / 0%; PGR-01 not armed
  - PGR-01 (1% / STAGE1_BOUNDARY_1PCT) path when explicitly flagged
  - Higher PGR gates (PGR-02+) remain locked / no unlock
...
# ---------------------------------------------------------------------------
# Higher gates locked / no PGR-02+
# ---------------------------------------------------------------------------


def test_phase5_pgr02_enable_does_not_unlock():
    _clear_flags()
    os.environ["AURORA_PGR_01_ENABLE"] = "1"
    ...
    try:
        ...
        # Higher gate attempt fail-closes stage-1 live path this mission
        assert get_funnel_pct() == 0
        assert boundary_funnel_enabled() is False
        assert pgr_flag_snapshot()["pgr02_not_started"] is True
    finally:
        _clear_flags()


def test_phase5_higher_pct_still_blocked_without_po_unlock():
    _clear_flags()
    os.environ["AURORA_PGR_01_ENABLE"] = "1"
