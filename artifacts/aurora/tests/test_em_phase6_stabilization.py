...
Proves:
  ...
  - Mirror drift OPEN and honestly documented (R-EM-01 / Plan IO9)
...
def test_stabilization_observability_sufficient():
    ...
    assert pgr["mirror_drift_open"] is True


def test_stabilization_mirror_drift_documented_open():
    _clear_em_flags()
    ...
    # Expected OPEN until Mission 044 closes R-EM-01 / Plan IO9.
    assert probe["mirror_drift_open"] is True
    assert len(probe["missing_in_mirror"]) > 0
    snap = em_flag_snapshot()
    assert snap["mirror_drift_open"] is True
    assert snap["em_mirror_drift"]["mirror_drift_open"] is True
