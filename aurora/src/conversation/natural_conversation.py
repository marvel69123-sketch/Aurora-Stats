...
def detect_natural_intent(message: str) -> dict[str, Any] | None:
    ...
    # CALENDAR-001: date offset via calendar_time (legacy regex or dateparser).
    _calendar_cue = re.search(
        r"\b(tem\s+jogo|jogo\s+d[oe]|jogos?\s+d[oe]|proximo\s+jogo|"
        r"quero\s+(?:saber\s+)?(?:sobre\s+)?(?:o\s+)?jogo|"
    ...
    if not _calendar_cue and not _is_recent_match_opinion(folded):
        # When ENABLE_DATEPARSER=1, also accept "joga domingo" / weekday relatives.
        try:
            from src.conversation.calendar_time import (
                dateparser_enabled,
                has_resolvable_calendar_date,
            )

            if dateparser_enabled() and has_resolvable_calendar_date(message or ""):
                if re.search(r"\b(?:joga|jogam)\b", folded) or re.search(
                    r"\b\w+\s+[xX]\s+\w+\b", message or ""
                ):
                    _calendar_cue = True
        except Exception:
    ...
    if not _is_recent_match_opinion(folded) and _calendar_cue:
        # Prefer pair
        m_pair = re.search(
        ...
        if teams:
            ...
            date_label = None
            try:
                from src.conversation.calendar_time import resolve_fixture_date

                hint = resolve_fixture_date(message or "")
                if hint is not None:
                    offset = int(hint.date_offset)
                    date_iso = hint.date_iso
                    date_label = hint.label
                elif re.search(r"\bamanha\b", folded):
                    # Fail-open parity with pre-CALENDAR-001 when no hint
                    offset = 1
            except Exception:
...
async def try_natural_conversation(
    ...
    try:
        ...
        if kind == "capabilities":
            ...
            # CALENDAR-001 — prefer ISO/label from calendar_time when present
            date_iso = str(detected.get("date_iso") or "") or _target_date(offset)
            label = str(detected.get("date_label") or "") or (
                "hoje" if offset == 0 else "amanhã"
            )
