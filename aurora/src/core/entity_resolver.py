...
def fuzzy_correct_team(
    ...
    *,
    ...
    ENTITY-EDGE-001: when ENABLE_ENTITY_EDGE=1, prefers RapidFuzz + league
    disambiguation (fail-open to this legacy path).
    """
    ...
    if exact:
        # League-aware override for bare collisions (Barcelona / Atlético)
        try:
            from src.core.entity_edge import maybe_override_alias

            overridden = maybe_override_alias(
                raw, exact, message=message, league_hint=league_hint
            )
            if overridden:
    ...
    # ENTITY-EDGE-001 — RapidFuzz path (flag default OFF)
    try:
        from src.core.entity_edge import edge_fuzzy_correct, entity_edge_enabled

        if entity_edge_enabled():
            edge_hit, edge_score = edge_fuzzy_correct(
                raw, message=message, league_hint=league_hint
            )
            if edge_hit:
                logger.warning(
                    "[DEBUG] fixture_resolver=entity_edge entity_match_score=%.3f "
                    "query=%r → %r",
                    edge_score, raw, edge_hit,
                )
                return edge_hit, round(float(edge_score), 3)
    except Exception:
        pass
    ...
    *,
    ...
    if exact:
        try:
            ...
            overridden = maybe_override_alias(
                name, exact, message=message, league_hint=league_hint
            )
            if overridden:
                logger.warning(
                    "[AUDIT] entity_resolver.normalize: %r → %r (edge_league)",
                    name, overridden,
                )
                return overridden
        except Exception:
    ...
    fuzzy_hit, fuzzy_score = fuzzy_correct_team(
        name, message=message, league_hint=league_hint
    )
    if fuzzy_hit:
        logger.warning(
