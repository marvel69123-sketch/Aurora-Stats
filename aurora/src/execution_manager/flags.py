...
def assess_em_mirror_drift() -> dict[str, Any]:
    ...
    Sync is NOT performed here — Mission 047 runs one-way
    ``scripts/sync-aurora-mirror.sh`` (SoT → mirror). Deploy SoT remains
    ``artifacts/aurora/``. Open drift ⇒ definitive Activation NO-GO.
    """
    ...
    return {
        ...
        "sync_deferred_reason": (
            "EM package absent/incomplete in aurora/ mirror — run "
            "bash scripts/sync-aurora-mirror.sh (Mission 047). Deploy SoT "
            "remains artifacts/aurora/."
            if open_drift
            else ""
        ),
        "disposition": "OPEN" if open_drift else "RESOLVED",
        "disposition_mission": "047",
...
def em_flag_snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "phase5_pgr02": True,
        "phase5_pgr02_defaults_off": not _flag_truthy("ENABLE_EM_PGR_02"),
        "phase5_pgr02_not_started": False,
        "phase5_pgr03": True,
        "phase5_pgr03_defaults_off": not _flag_truthy("ENABLE_EM_PGR_03"),
        "phase5_pgr03_not_started": False,
        "phase5_pgr04": True,
        "phase5_pgr04_defaults_off": not _flag_truthy("ENABLE_EM_PGR_04"),
        "phase5_pgr04_not_started": False,
        "phase5_pgr05": True,
...
def rollback_em_pgrXX_to_off(gate: int) -> None:
    ...
    if gate == 1:
        ...
        return
    if gate == 2:
        from src.execution_manager.progressive_gate import rollback_em_pgr02_to_off

        rollback_em_pgr02_to_off()
        return
    if gate == 3:
        from src.execution_manager.progressive_gate import rollback_em_pgr03_to_off

        rollback_em_pgr03_to_off()
        return
    if gate == 4:
        from src.execution_manager.progressive_gate import rollback_em_pgr04_to_off

        rollback_em_pgr04_to_off()
        return
    if gate == 5:
        from src.execution_manager.progressive_gate import rollback_em_pgr05_to_off
